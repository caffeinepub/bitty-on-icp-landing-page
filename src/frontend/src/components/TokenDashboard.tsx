...
export default function TokenDashboard() {
    ...
    useEffect(() => {
        const fetchDexscreenerData = async () => {
            try {
                const response = await fetch('https://api.dexscreener.com/latest/dex/pairs/icp/wkyqn-qqaaa-aaaar-qbyxq-cai');
        ...
        >
            ...
            <div className="relative z-10 max-w-7xl mx-auto">
                            ...
                            <>
                                ...
                                <div className="relative group">
                                    ...
                                    <div className="relative bg-gradient-to-br from-purple-900/90 to-indigo-900/90 backdrop-blur-sm rounded-2xl p-6 border-4 border-neon-orange shadow-neon-orange">
                                        <h3 className="text-xl font-bold text-white/80 drop-shadow-glow-white mb-2">Current Price</h3>
                                        <p className="text-4xl font-black text-white drop-shadow-glow-orange-intense">
                                            {formatPrice(dexData.priceUsd)}
                                        </p>
                                        <p className="text-xs text-white/50 mt-2">Live from Dexscreener</p>
                                    </div>
                                </div>
                                ...
                                <div className="relative group">
                                    <div className="absolute inset-0 bg-neon-orange rounded-2xl blur-xl opacity-60 group-hover:opacity-80 transition-opacity duration-300" />
                                    <div className="relative bg-gradient-to-br from-purple-900/90 to-indigo-900/90 backdrop-blur-sm rounded-2xl p-6 border-4 border-neon-orange shadow-neon-orange">
                                        <h3 className="text-xl font-bold text-white/80 drop-shadow-glow-white mb-2">24h Volume</h3>
                                        <p className="text-4xl font-black text-white drop-shadow-glow-orange-intense">
                                            {formatNumber(dexData.volume?.h24)}
                                        </p>
                                        <p className="text-xs text-white/50 mt-2">Trading volume last 24 hours</p>
                                    </div>
                                </div>
                                ...
                                <div className="relative group">
                                    <div className="absolute inset-0 bg-neon-orange rounded-2xl blur-xl opacity-60 group-hover:opacity-80 transition-opacity duration-300" />
                                    <div className="relative bg-gradient-to-br from-purple-900/90 to-indigo-900/90 backdrop-blur-sm rounded-2xl p-6 border-4 border-neon-orange shadow-neon-orange">
                                        <h3 className="text-xl font-bold text-white/80 drop-shadow-glow-white mb-2">Market Cap</h3>
                                        <p className="text-4xl font-black text-white drop-shadow-glow-orange-intense">
                                            {formatNumber(dexData.marketCap || dexData.fdv)}
                                        </p>
                                        <p className="text-xs text-white/50 mt-2">Fully Diluted Valuation</p>
                                    </div>
                                </div>
                            </>
                        )}
                ...
                <div className="mt-8 text-center">
                    <p className="text-sm text-white/60 drop-shadow-glow-white">
                        Live data from Dexscreener • Auto-refreshes every 30 seconds • Token Pair: wkyqn-qqaaa-aaaar-qbyxq-cai
                    </p>
                </div>
            </div>
        </section>
