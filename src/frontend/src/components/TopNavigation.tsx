...
export default function TopNavigation() {
    ...
    const scrollToSection = (sectionId: string) => {
        ...
        // Always close dropdown, even if element not found
        setIsOpen(false);
    };
    ...
    const menuItems = [
        ...
        { label: 'TOKEN DASHBOARD', sectionId: 'token-dashboard', type: 'scroll' as const },
        { label: 'BITTYICP BLING', type: 'comingSoon' as const },
        { label: 'MEME PAGE', url: 'https://bittyicpmeme-ebv.caffeine.xyz/', type: 'external' as const },
    ...
    const handleMenuItemClick = (item: typeof menuItems[0]) => {
        ...
        // contractId type is handled by the button's own click handler
    };
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex items-center justify-center h-16 sm:h-20">
                        ...
                        <div className="relative">
                            ...
                            >
                                <span className="drop-shadow-glow-orange-intense relative z-10">A BITTY MORE</span>
                                <ChevronDown className={`h-5 w-5 sm:h-6 sm:w-6 transition-transform duration-300 relative z-10 ${isOpen ? 'rotate-180' : ''}`} />
                                                        ...
                                                        >
                                                            ...
                                                            {copied ? (
                                                                <>
                                                                    ...
                                                                    <span className="relative z-10 text-xs whitespace-nowrap drop-shadow-glow-orange">Copied!</span>
                                                                </>
                                                            ) : (
                                                                <>
                                                                    <Copy className="h-4 w-4 relative z-10" />
                                                                    <span className="relative z-10 text-xs whitespace-nowrap drop-shadow-glow-orange">Copy</span>
                                                                </>
                                                            )}
                                                        </button>
                                                    </div>
